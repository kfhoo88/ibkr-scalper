...
class IBKRSyncDownloader:
    ...
    def download_chunked_data(self, symbol, days_per_chunk=30, total_days=60):
        """
        Download data in smaller chunks synchronously
        """
        ...
        for chunk in range(0, total_days, days_per_chunk):
            ...
            try:
                ...
                # Request 5-minute data for this chunk (more reliable than 1-min)
                bars = self.ib.reqHistoricalData(
                    contract,
                    endDateTime=chunk_end.strftime('%Y%m%d %H:%M:%S'),
                    durationStr=f'{days_per_chunk} D',
                    barSizeSetting='5 min',  # Using 5-min for reliability
                    whatToShow='TRADES',
                    useRTH=True,
                    formatDate=1,
                    timeout=60
    ...
    def download_with_fallback(self, symbol, total_days=60):
        ...
        # Try 30-day chunks first with 5-min data
        ...
        if data.empty:
            print("🔄 Falling back to 7-day chunks...")
            data = self.download_chunked_data(symbol, days_per_chunk=7, total_days=min(total_days, 30))
    ...
    def run(self):
        ...
        for symbol in symbols:
            try:
                # Start with 60 days of 5-min data (more reliable)
