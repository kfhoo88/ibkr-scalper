# vwap_ma_strategy/main_reversal_detailed_with_analysis_fixed.py
"""
Reversal Strategy - FIXED VERSION 
Same profitable logic as original + enhanced analysis features
"""
...
class ReversalStrategyDetailedFixed:
    def __init__(self, config):
        ...
        # NEW: Configurable position sizing from original
        ...
        print("🔧 REVERSAL STRATEGY - FIXED VERSION")
        print(f"   Original profitable logic + Enhanced analysis")
        print(f"   Position Size: {self.position_size}")
    
    def load_data_proper(self, symbol):
        """Load data with proper timezone handling - SAME AS ORIGINAL"""
    ...
    def is_valid_trading_time(self, timestamp):
        """Check if current time is valid for trading - SAME AS ORIGINAL"""
    ...
    def should_force_exit(self, timestamp, entry_time, current_bar):
        """Check if we should force exit - SAME AS ORIGINAL"""
        time_only = timestamp.tz_convert(self.est).tz_localize(None).time()
    ...
    def calculate_indicators(self, df):
        """Calculate EMA and swing points - SAME AS ORIGINAL"""
    ...
    def generate_signals(self, df):
        ...
        for i in range(self.ema_backcandles + 1, len(df)-1):
            ...
            if (current_ema == 2 and 
                ...
                df.loc[df.index[i+1], 'SL'] = swing_low_price - (current_atr * self.sl_atr_multiplier)
                df.loc[df.index[i+1], 'TP'] = entry_price + ((entry_price - df['SL'].iloc[i+1]) * self.tp_multiplier)
            
            # SHORT ENTRY - EXACT ORIGINAL
                ...
                df.loc[df.index[i+1], 'Entry_Price'] = entry_price
                df.loc[df.index[i+1], 'SL'] = swing_high_price + (current_atr * self.sl_atr_multiplier)
