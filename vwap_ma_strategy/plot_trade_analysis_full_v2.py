...
class TradeVisualizerFull:
    ...
    def load_trade_data(self, symbol):
        """Load saved trade data - NUCLEAR OPTION"""
        filename = f'trade_data_{symbol}.pkl'
        try:
            trades_df = pd.read_pickle(filename)
            print(f"✅ Loaded {len(trades_df)} {symbol} trades")
            
            # NUCLEAR OPTION: Convert to string and back to datetime to strip ALL timezone info
            trades_df['entry_time'] = pd.to_datetime(trades_df['entry_time'].astype(str))
            trades_df['exit_time'] = pd.to_datetime(trades_df['exit_time'].astype(str))
    ...
    def load_price_data(self, symbol):
        """Load historical price data - ORIGINAL WORKING VERSION"""
    ...
    # All the original methods stay unchanged
