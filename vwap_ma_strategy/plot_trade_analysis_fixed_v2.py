# vwap_ma_strategy/plot_trade_analysis_fixed_v2.py
"""
Fixed Trade Visualization - Handle both timezone-aware and naive timestamps
"""
...
class TradeVisualizerFixedV2:
    ...
    def load_trade_data(self, symbol):
        """Load saved trade data and ensure proper datetime format"""
        ...
        try:
            ...
            print(f"   Entry time type: {type(trades_df['entry_time'].iloc[0])}")
            print(f"   Exit time type: {type(trades_df['exit_time'].iloc[0])}")
    ...
    def load_price_data(self, symbol):
        ...
        try:
            ...
            # Handle timezone properly - remove timezone info
        ...
        for symbol in ['SPY']:  # Start with just SPY for testing
            ...
            for i, (idx, trade) in enumerate(test_trades.iterrows()):
                ...
                if fig:
                    ...
                    print(f"    Trade details: Entry=${trade['entry_price']:.2f}, Exit=${trade['exit_price']:.2f}")
                else:
                    print(f"    ❌ Failed to create chart for trade {i+1}")

def run_fixed_visualization_v2():
    """Run the fixed visualization with proper time alignment"""
    print("🎯 FIXED TRADE VISUALIZATION V2 - PROPER TIME ALIGNMENT")
    print("Handling both timezone-aware and naive timestamps")
    print("=" * 60)
    ...
    visualizer = TradeVisualizerFixedV2(config)
    visualizer.generate_fixed_charts(max_trades=2)

if __name__ == "__main__":
    run_fixed_visualization_v2()